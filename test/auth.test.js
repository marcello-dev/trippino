...
describe('Authentication API', () => {
  ...
  describe('POST /api/signup', () => {
    it('should create a new user account', async () => {
      ...
      const user = await get(`SELECT * FROM users WHERE email = ?`, ['newuser@example.com']);
  ...
  describe('POST /api/verify-email', () => {
    it('should verify email with valid token', async () => {
      ...
      const user = await get(`SELECT * FROM users WHERE email = ?`, ['user@example.com']);
  ...
  describe('POST /api/change-password', () => {
    it('should change password and invalidate all sessions', async () => {
      ...
      const sessions = await all(`SELECT * FROM sessions WHERE user_id = ?`, [user.id]);
      expect(sessions.length).toBe(0);
    });
    
    it('should reject wrong current password', async () => {
      const user = await createTestUser(run, 'user@example.com', 'correctpassword');
      const sid = await createTestSession(run, user.id);
