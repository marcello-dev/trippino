...
describe("City API", () => {
  beforeEach(async () => {
    ...
    app.delete("/api/trips/:id/cities/:cityId", async (req, res) => {
      try {
        ...
        await run(`DELETE FROM cities WHERE id = ?`, [req.params.cityId]);
  ...
  describe("POST /api/trips/:id/cities", () => {
    it("should create a city with auto sort order", async () => {
      ...
      const city = await get(`SELECT * FROM cities WHERE id = ?`, [
        response.body.id,
      ]);
  ...
  describe("PUT /api/trips/:tripId/cities (batch sort order)", () => {
    it("should update sort order for multiple cities", async () => {
      ...
      const response = await request(app)
        ...
        .set("Cookie", `trippino_sid=${sid}`)
        .send({
          sortOrder: [{ id: city3.id }, { id: city2.id }, { id: city1.id }],
        });

      expect(response.status).toBe(200);

      const updated1 = await get(`SELECT * FROM cities WHERE id = ?`, [
        city1.id,
      ]);
      const updated2 = await get(`SELECT * FROM cities WHERE id = ?`, [
        city2.id,
      ]);
      const updated3 = await get(`SELECT * FROM cities WHERE id = ?`, [
        city3.id,
      ]);
  ...
  describe("DELETE /api/trips/:id/cities/:cityId", () => {
    ...
    it("should validate trip ownership", async () => {
      ...
      const stillExists = await get(`SELECT * FROM cities WHERE id = ?`, [
        city2.id,
      ]);
    ...
    it("should require authentication", async () => {
      ...
      const response = await request(app).delete(
        `/api/trips/${trip.id}/cities/${city.id}`,
      );
